...
# Função para encontrar o subconjunto de códons que geram o peptídeo mais longo
def encontrar_orf_longest(sequence):
    orf = []
    for i in range(0, len(sequence) - len(sequence) % 3, 3):
        codon = sequence[i:i+3]
        orf.append(codon)
        # Verifica se encontrou um codão de parada
...
arquivo_orf_longest = "Python_08.orf-longest.nt"
...
try:
    ...
    with open(arquivo_codons, "w") as saida_codons, open(arquivo_traduzido, "w") as saida_traduzido, open(arquivo_peptideo, "w") as saida_peptideo, open(arquivo_orf_longest, "w") as saida_orf_longest:
        # Para cada sequência, gere os 6 quadros de leitura
        for seq_name, sequence in sequencias.items():
            # Variáveis para armazenar o peptídeo mais longo e o ORF correspondente
            longest_peptide = ""
            longest_orf = ""
            ...
            for frame in range(3):
                ...
                if len(peptideo_longest) > len(longest_peptide):
                    longest_peptide = peptideo_longest
                    longest_orf = encontrar_orf_longest(sequence[frame:])

                # Grava o peptídeo mais longo no arquivo
                saida_peptideo.write(f">{seq_name}-frame-{frame+1}-longest-peptideo\n")
                saida_peptideo.write(longest_peptide + "\n")
            ...
            for frame in range(3):
                ...
                if len(peptideo_longest) > len(longest_peptide):
                    longest_peptide = peptideo_longest
                    longest_orf = encontrar_orf_longest(complement_sequence[frame:])

                # Grava o peptídeo mais longo no arquivo
                saida_peptideo.write(f">{seq_name}-frame-{frame+4}-longest-peptideo\n")
                saida_peptideo.write(longest_peptide + "\n")

            # Grava o subconjunto de códons do ORF mais longo no arquivo
            saida_orf_longest.write(f">{seq_name}-longest-orf\n")
            saida_orf_longest.write(longest_orf + "\n")

            saida_codons.write("\n")  # Pula uma linha entre as sequências
            saida_traduzido.write("\n")  # Pula uma linha entre as sequências
            saida_peptideo.write("\n")  # Pula uma linha entre as sequências
            saida_orf_longest.write("\n")  # Pula uma linha entre as sequências
    ...
    print(f"O ORF mais longo foi gravado no arquivo '{arquivo_orf_longest}'.")
